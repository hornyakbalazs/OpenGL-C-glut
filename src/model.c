...
GLuint load_texture(const char* filename) {
	glPixelStorei(GL_UNPACK_ALIGNMENT,1); //pixel tárolási módok beállítása
	GLuint texture_name;
	Pixel* image;
	...
	glTexEnvf(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_MODULATE); //beállítja a textúra környezeti paramétereit
...
void init_entities(World* world) {
	// Dino1
	load_model("models//dino1.obj", &world->dino1.model);
	world->dino1.texture = load_texture("textures//dino1.png");
	init_object(&world->dino1, 100, 100);

	// Dino2
	load_model("models//dino2.obj", &world->dino2.model);
	world->dino2.texture = load_texture("textures//dino2.png");
	init_object(&world->dino2, -100, 100);
