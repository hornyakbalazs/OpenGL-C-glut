...
GLfloat material_ambient_default[] = {0.15, 0.15, 0.15, 1.0};
GLfloat material_diffuse_default[] = {0.6, 0.6, 0.6, 1.0};
GLfloat material_specular_default[] = {0.9, 0.9, 0.9, 1.0};
...
void draw_content(World* world)
{
	...
	glPushMatrix();	
		...
		glMaterialfv(GL_FRONT_AND_BACK, GL_DIFFUSE, material_diffuse_default);
		glMaterialfv(GL_FRONT_AND_BACK, GL_SPECULAR, material_specular_default);
       	glMaterialfv(GL_FRONT_AND_BACK, GL_SHININESS, &shineness);

		glBindTexture(GL_TEXTURE_2D, world->dino1.texture);
		
		glTranslatef(world->dino1.position.x, world->dino1.position.y, world->dino1.position.z);
	...
	glPushMatrix();	
		...
		glBindTexture(GL_TEXTURE_2D, world->dino2.texture);
		
		glTranslatef(world->dino2.position.x, world->dino2.position.y, world->dino2.position.z);
